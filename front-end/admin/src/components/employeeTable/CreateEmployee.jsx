...
const CreateEmployee = () => {
  ...
  const [photos, setPhotos] = useState([]); // Menggunakan array untuk menyimpan foto yang akan diunggah
  ...
  const cleanFormData = () => {
    ...
    setStatus("");
    setJob("");
    setPhotos([]); // Mengosongkan array foto saat membersihkan data formulir
  };
  ...
  const handleCreate = async () => {
    ...
    if (!response.ok) {
      ...
      const employeeData = await response.json(); // Mengambil data respons JSON dengan ID pegawai
  
      const formData = new FormData();
      photos.forEach((photo) => {
        const photoName = `${employeeData.id}.jpg`; // Menggunakan ID pegawai sebagai nama file
